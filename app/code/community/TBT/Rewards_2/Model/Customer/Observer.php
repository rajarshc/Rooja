...
class TBT_Rewards_Model_Customer_Observer extends Varien_Object
{
	...
	{
	    $customer = Mage::getModel('rewards/customer')->getRewardsCustomer(  $observer->getEvent()->getDataObject()  );
		...
		if ($isNew) {
			if (Mage::helper('rewards/dispatch')->smartDispatch('rewards_customer_signup', array('customer' => $customer))) {
