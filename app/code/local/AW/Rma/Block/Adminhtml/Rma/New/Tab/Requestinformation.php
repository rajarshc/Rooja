...
class AW_Rma_Block_Adminhtml_Rma_New_Tab_Requestinformation extends Mage_Adminhtml_Block_Widget_Form {
    protected function _prepareForm() {
        ...
        $details = $form->addFieldset('order_form', array(
            'legend' => $this->__('Order Details')
        ));

        $details->addField('order', 'text', array(
            'name' => 'order',
            'label' => $this->__('Order ID'),
            'class' => 'awrma-tip',
            'required'  => true,
            'value' =>  'start type order\'s id',
            'after_element_html' => '<div id="orders-list"><select id="order-selector"></select></div>'
        ));
    }
}
