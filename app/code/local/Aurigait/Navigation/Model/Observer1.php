...
class Aurigait_Navigation_Model_Observer 
{
	...
	public function disableProducts()
	{
		...
		$sql="select entity_id from  catalog_category_flat_store_1 where entity_id>990 and date(sale_end_date)!='0000-00-00' and (date(sale_end_date)<'".date('y-m-d')."' or (date(sale_end_date)='".date('y-m-d')."' and sale_end_time<'".date('H:i:00',strtotime('+330 minutes'))."'))";
	...
	{
		$read= Mage::getSingleton('core/resource')->getConnection('core_read');
		$sql="select entity_id from  catalog_category_flat_store_1 where entity_id in (".$cat_ids.") and (date(sale_end_date)<'".date('Y-m-d')."' or (date(sale_end_date)='".date('Y-m-d')."' and sale_end_time < '".date('H:i:00',strtotime('+330 minutes'))."'))";
		$results=$read->fetchAll($sql);
		if(count($results)>0)
		 return false;
		else
